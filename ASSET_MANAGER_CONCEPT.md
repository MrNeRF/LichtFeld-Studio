...
## 16. Corrections from the Codex astra critique of v2 (folded 2026-09-14, `critique_codex/report.md`)

- Replacement never discards or moves a journal link. The new entry at the path records the previous project id as a reference; the link stays under the old id until an explicit replacement upload succeeds, and that handoff is one journal transaction with restart recovery (F01). If the old id turns up at another path in the same reconciliation, it is relocated and keeps its link; the new file is an ordinary new project.
- Reconciliation is order independent: observations first, resolution second (F04). Aliases carry file and commit ids; divergent copies are a flagged state that blocks Publish and Trash until the user chooses.
- Cache invalidation uses stat identity (size, mtime, device, inode, ctime); explicit Verify, reconciliation and every mutation preflight re-read the head and compare ids (F08).
- Catalog schema 6 for the new fields (name provenance, previous project id, aliases, stat identity, versioned inspection facts) with a one-time `library.json.v5.bak`; unknown fields are preserved (F06). Name provenance is migrated (stem or user) and one `display_name` function serves the panel, the startup chooser and Open Recent (F12). The PROJ title stays a JSON addition without a container version bump until the portal's portable validator accepts it.
- "Gallery not checked" is a real state: before an account-matched journal snapshot exists, a linked entry is never shown as Not published (F05). "Linked, comparison unavailable" persists when an exchange baseline is missing.
- Apply gallery changes is split: settings-only apply (title, description, visibility, view settings) never enters the geometry replacement path; a geometry apply names the checkpoint loss and defaults to keeping local geometry when it is checkpoint-backed (F02). Content-revision changes on the portal are shown as "Changes in gallery (scene content)" separately from metadata.
- Reduce size may drop embedded inputs only when a validated external replacement exists for every required embedded input (images, normals, sparse data), with a preview of the exact removal set, a recovery copy and a post-check (F03). The owner's embedded file holds 194 images, 194 normals and 3 sparse files, not 391 images (F15).
- Save history shows retained checkpoints separately from container saves; the SCNG-bound checkpoint is protected; Compact warns that older save points disappear; "Open as new project" mints a new identity; "Resume from here" rebinds explicitly and keeps a recovery copy (F09).
- The card inspection tier is provisional: head-only facts, not a certification that the file opens; authoritative health arrives after off-thread structural validation; under 1 ms is a warm target (F07).
