...
namespace tinyply {
    ...
    struct PlyProperty {
        ...
        PlyProperty(Type type, std::string& _name) : name(_name),
                                                     propertyType(type) {}
    ...
    struct PlyElement {
        ...
        PlyElement(const std::string& _name, size_t count) : name(_name),
                                                             size(count) {}
        std::string name;
        size_t size{0};
...
namespace tinyply {
    ...
    inline T2 endian_swap(const T& v) noexcept { return v; }
    template <>
    inline uint16_t endian_swap<uint16_t, uint16_t>(const uint16_t& v) noexcept { return (v << 8) | (v >> 8); }
    template <>
    inline uint32_t endian_swap<uint32_t, uint32_t>(const uint32_t& v) noexcept { return (v << 24) | ((v << 8) & 0x00ff0000) | ((v >> 8) & 0x0000ff00) | (v >> 24); }
    template <>
    ...
    void PlyFile::PlyFileImpl::write_property_binary(std::ostream& os, const uint8_t* src, size_t& srcOffset, const size_t& stride) noexcept {
    ...
    void PlyFile::PlyFileImpl::read(std::istream& is) {
        ...
        for (auto& entry : userData)
            buffers.push_back(entry.second.data);
