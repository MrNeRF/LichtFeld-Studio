...
namespace gsplat_lfs {
    ...
    __global__ void rasterize_to_pixels_from_world_3dgs_fwd_kernel_tam(
        const uint32_t C,
        const uint32_t N,
        ...
        // Taming3DGS-Plus specifics
        const float* __restrict__ pixel_weights, // [image_height, image_width]
        float* __restrict__ accum_weights        // [N]
    ) {
...
#pragma unroll
        ...
        //int32_t* pixel_count_batch =
        //    reinterpret_cast<int32_t*>(&iscl_rot_batch[block_size]); // [block_size]
        ...
        for (uint32_t b = 0; b < num_batches; ++b) {
            ...
            for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
                ...
                const vec2 d = {xyz[0] - px, xyz[1] - py};
...
#pragma unroll
                ...
                // Taming retrieval
             //   atomicAdd(&blend_weights[id_batch[t]], T * alpha);
             //   atomicAdd(&pixel_count_batch[t], 1); // sh

                atomicAdd(&accum_weights[id_batch[t]], pixel_weights[pix_id]);
             //   atomicAdd(&dist_accum[id_batch[t]], sqrt(d[0] * d[0] + d[1] * d[1]));
...
#pragma unroll
    ...
    void launch_rasterize_to_pixels_from_world_3dgs_fwd_kernel_tam(
        ...
        int64_t shmem_size =
            tile_size * tile_size *
            (sizeof(int32_t) + sizeof(vec4) + sizeof(mat3) + sizeof(int32_t));
        ...
        auto err = cudaFuncSetAttribute(
            rasterize_to_pixels_from_world_3dgs_fwd_kernel_tam<CDIM, float>,
...
#define __INS__(CDIM)                                                          \
    template void launch_rasterize_to_pixels_from_world_3dgs_fwd_kernel_tam<CDIM>( \
        const float* means,                                                    \
        const float* quats,                                                    \
        const float* scales,                                                   \
        const float* colors,                                                   \
