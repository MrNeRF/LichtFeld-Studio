...
namespace lfs::io {
    ...
    namespace {
        ...
        std::expected<SplatData, std::string> reconstruct_splat_data(
            ...
            // Check if shN data is actually non-zero
            float shN_min = 0.0f, shN_max = 0.0f;
            if (shN_dim1 > 0 && !host_shN.empty()) {
                shN_min = *std::min_element(host_shN.begin(), host_shN.end());
                shN_max = *std::max_element(host_shN.begin(), host_shN.end());
            }
            
            LOG_INFO("Creating SplatData with sh_degree={}, shN tensor shape={}x{}x{}",
                     sh_degree, N, shN_dim1, shN_dim2);
            LOG_INFO("shN data range: min={}, max={} (non-zero: {})",
                     shN_min, shN_max, (shN_min != 0.0f || shN_max != 0.0f));
