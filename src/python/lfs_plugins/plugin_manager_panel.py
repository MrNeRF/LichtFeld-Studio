    ...
    def draw(self, layout):
        ...
        if layout.collapsing_header("Install from GitHub", default_open=True):
            ...
            if in_progress:
                ...
                if layout.button_styled("Install Plugin", "success", (top_button_w, 28)):
                    self._install_plugin(mgr)
                layout.same_line(spacing=8.0)
                if layout.button_styled("Explore Options", "primary", (top_button_w, 28)):
        ...
        if layout.collapsing_header("Installed Plugins", default_open=True):
            ...
            if not plugins:
                layout.text_colored("No plugins installed", palette.text_dim)
            else:
                ...
                if 0 <= self.selected_plugin_idx < len(plugins):
                    ...
                    if state == PluginState.ACTIVE:
                        layout.text_colored(f"Status: {state_str}", palette.success)
                    elif state == PluginState.ERROR:
                        layout.text_colored(f"Status: {state_str}", palette.error)
                    ...
                    if not in_progress:
                        if state == PluginState.ACTIVE:
                            if layout.button_styled("Reload", "primary"):
                                self._reload_plugin(mgr, plugin.name)
                            layout.same_line()
                            if layout.button_styled("Unload", "warning"):
                                self._unload_plugin(mgr, plugin.name)
                        else:
                            if layout.button_styled("Load", "success"):
                                self._load_plugin(mgr, plugin.name)

                        layout.same_line()
                        if layout.button_styled("Update", "primary"):
                            self._update_plugin(mgr, plugin.name)

                        layout.same_line()
                        if layout.button_styled("Uninstall", "error"):
