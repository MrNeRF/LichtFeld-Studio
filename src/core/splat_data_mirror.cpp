...
void ensure_cache(const Device device) {
    auto& c = get_cache();
    if (c.valid && c.device == device) return;
    ...
    for (int a = 0; a < 3; ++a) {
        ...
        for (int d = 0; d < 4; ++d) {
            ...
            for (int i = 0; i < n; ++i) v[i] = SH_MULT[a][i];
...
glm::vec3 compute_selection_center(const SplatData& splat_data, const Tensor& selection_mask) {
    ...
    if (!means.is_valid() || means.size(0) == 0) return glm::vec3(0.0f);
...
void mirror_gaussians(SplatData& splat_data,
    ...
    if (!means.is_valid() || means.size(0) == 0) return;
    ...
    const auto selected = selection_mask.ne(0);
    if (selected.sum_scalar() == 0) return;

    auto indices = selected.nonzero();
    if (indices.ndim() == 2) indices = indices.squeeze(1);
    if (indices.dtype() != DataType::Int32) indices = indices.to(DataType::Int32);
