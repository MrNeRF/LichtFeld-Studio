...
namespace lfs::vis::gui {
    ...
    void RmlStatusBar::updateTheme() {
        ...
        if (base_rcss_.empty()) {
            try {
                auto rcss_path = lfs::vis::getAssetPath("rmlui/statusbar.rcss");
                ...
                LOG_ERROR("RmlStatusBar: RCSS not found: {}", e.what());
            }
        }
    ...
    void RmlStatusBar::initFBO(int w, int h) {
        ...
        if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
            LOG_ERROR("RmlStatusBar: FBO incomplete");
