// SPDX-FileCopyrightText: 2008-2014 CodePoint Ltd, Shift Technology Ltd, and contributors
// SPDX-FileCopyrightText: 2019-2026 The RmlUi Team, and contributors
// SPDX-FileCopyrightText: 2026 LichtFeld Studio Authors
//
// SPDX-License-Identifier: MIT
//
// Adapted from the upstream RmlUi GL3 renderer backend. It is kept local to
// the visualizer so builds do not depend on a separate RmlUi source checkout.

#include "rmlui_gl3_backend.hpp"
...
enum class ShaderGradientFunction { Linear,
                                    Radial,
                                    Conic,
                                    RepeatingLinear,
                                    RepeatingRadial,
                                    RepeatingConic }; // Must match shader definitions below.
...
namespace Gfx {
    ...
    enum class VertexAttribute { Position,
                                 Color0,
                                 TexCoord0,
                                 Count };
...
static const ProgramDefinition program_definitions[] = {
    ...
    enum class FramebufferAttachment { None,
                                       DepthStencil };
    ...
    static void CheckGLError(const char* operation_name) {
        ...
        if (error_code != GL_NO_ERROR) {
            static const Rml::Pair<GLenum, const char*> error_names[] = {{GL_INVALID_ENUM, "GL_INVALID_ENUM"}, {GL_INVALID_VALUE, "GL_INVALID_VALUE"}, {GL_INVALID_OPERATION, "GL_INVALID_OPERATION"}, {GL_OUT_OF_MEMORY, "GL_OUT_OF_MEMORY"}};
...
void RenderInterface_GL3::EndFrame() {
    ...
    // Draw to target framebuffer (0 = backbuffer, or caller-specified FBO)
...
enum class FilterType { Invalid = 0,
                        Passthrough,
                        Blur,
                        DropShadow,
                        ColorMatrix,
