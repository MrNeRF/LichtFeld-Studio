...
namespace lfs::vis::tools {
    ...
    bool SelectionTool::handleKeyPress(const int key, const int mods, const ToolContext& ctx) {
        ...
        if (key == GLFW_KEY_F && (mods & GLFW_MOD_CONTROL) && (mods & GLFW_MOD_SHIFT)) {
            auto* const sm = ctx.getSceneManager();
            if (sm) {
                const auto selected_type = sm->getSelectedNodeType();
                if (selected_type == NodeType::CROPBOX || selected_type == NodeType::ELLIPSOID) {
                    const auto& visible = sm->getScene().getVisibleNodes();
    ...
    bool SelectionTool::isCropBoxSelected() const {
        if (!tool_context_)
            return false;
        ...
        const auto selected_type = sm->getSelectedNodeType();
        return selected_type == NodeType::CROPBOX || selected_type == NodeType::ELLIPSOID;
    }
