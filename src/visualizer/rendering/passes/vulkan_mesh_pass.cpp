...
namespace lfs::vis {
    ...
    struct VulkanMeshPass::Impl {
        ...
        void record(VkCommandBuffer cb, VkExtent2D extent, const VulkanMeshPassParams& params) {
    ...
    void VulkanMeshPass::record(VkCommandBuffer command_buffer, VkExtent2D extent,
                                const VulkanMeshPassParams& params) {
        if (!impl_)
            return;
        impl_->record(command_buffer, extent, params);
    }
