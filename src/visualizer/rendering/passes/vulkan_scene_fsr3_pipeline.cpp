...
namespace lfs::vis {
    ...
    struct VulkanSceneFsr3Pipeline::Impl {
        ...
        [[nodiscard]] bool ensureRuntime() {
            ...
            runtime_initialized = AmdFsr3Plugin::instance().initializeRuntime(config);
        ...
        void destroyOutput(OutputResource& output) {
            if (!output.vram_label.empty()) {
                lfs::diagnostics::VramProfiler::instance().recordCurrentBytes(
                    "vulkan.scene_fsr3.output", output.vram_label, 0);
            }
        ...
        void destroy() {
            for (std::size_t index = 0; index < views.size(); ++index) {
                AmdFsr3Plugin::instance().releaseFeature(
        ...
        [[nodiscard]] bool createOutput(OutputResource& output,
            ...
            if (!vk_try_bool(vmaCreateImage(allocator,
                                            ...
                                            &output.allocation,
                                            &allocation_result),
                             "vmaCreateImage(scene_fsr3.output)",
                             "FSR 3.1 output allocation failed"))
                return false;
            ...
            if (!vk_try_bool(vkCreateImageView(device, &view_info, nullptr, &output.view),
                             "vkCreateImageView(scene_fsr3.output)",
                             "FSR 3.1 output view creation failed")) {
                destroyOutput(output);
                return false;
            }
            ...
            lfs::diagnostics::VramProfiler::instance().recordCurrentBytes(
                "vulkan.scene_fsr3.output", output.vram_label, output.allocation_bytes);
            context->setDebugObjectName(
                VK_OBJECT_TYPE_IMAGE, output.image, "scene_fsr3.output");
            context->setDebugObjectName(
                VK_OBJECT_TYPE_IMAGE_VIEW, output.view, "scene_fsr3.output.view");
            return true;
        }
        ...
        [[nodiscard]] bool ensureFeature(const VkCommandBuffer command_buffer,
            ...
            if (changed && view.feature_configured && !context->waitForSubmittedFrames())
                return false;
            if (!AmdFsr3Plugin::instance().createFeature(command_buffer, feature))
                return false;
            if (changed)
        ...
        [[nodiscard]] VulkanSceneFsr3PipelineResult fail(
            const PreparedSceneTemporalFrame& prepared,
        ...
        [[nodiscard]] VulkanSceneFsr3PipelineResult record(
            ...
            if (!validVulkanSceneFsr3PipelineRequest(request) ||
                ...
                return {.status = VulkanSceneFsr3PipelineStatus::InvalidRequest,
            ...
            if (!motion_initialized)
                return fail(prepared,
                            VulkanSceneFsr3PipelineStatus::MotionUnavailable,
            ...
            if (!motion.record(command_buffer, motion_params, *resource_slot))
                return fail(prepared,
                            VulkanSceneFsr3PipelineStatus::MotionFailure,
            ...
            if (!depth_initialized)
                return fail(prepared,
                            VulkanSceneFsr3PipelineStatus::DepthUnavailable,
                            TemporalResetReason::RuntimeUnavailable);
            ...
            if (!depth.record(command_buffer, depth_params, *resource_slot))
                return fail(prepared,
                            VulkanSceneFsr3PipelineStatus::DepthFailure,
                            TemporalResetReason::ResolveFailure);

            auto* const output = ensureOutput(*resource_slot, prepared.plan.output_extent);
            if (output == nullptr)
                return fail(prepared,
                            VulkanSceneFsr3PipelineStatus::OutputFailure,
                            TemporalResetReason::ResolveFailure);
            if (!ensureFeature(command_buffer, prepared, request.quality))
                return fail(prepared,
                            VulkanSceneFsr3PipelineStatus::FeatureFailure,
                            TemporalResetReason::RuntimeUnavailable);
            ...
            const LfsSceneUpscalerEvaluateV1 evaluation{
                ...
                // LichtFeld has no canonical scene-unit-to-metre scale. Pass the
                // neutral FidelityFX scale explicitly instead of inventing one.
                .view_space_to_meters = 1.0f,
            };
            if (!AmdFsr3Plugin::instance().evaluate(evaluation))
                return fail(prepared,
                            VulkanSceneFsr3PipelineStatus::EvaluateFailure,
                            TemporalResetReason::ResolveFailure);
            ...
            if (!coordinator.commit(prepared,
                                    ...
                                    SceneHistoryStorage::VulkanImage))
                return fail(prepared,
                            VulkanSceneFsr3PipelineStatus::CommitFailure,
                            TemporalResetReason::ResolveFailure);
    ...
    VulkanSceneFsr3Pipeline::VulkanSceneFsr3Pipeline() = default;
    VulkanSceneFsr3Pipeline::~VulkanSceneFsr3Pipeline() = default;
    VulkanSceneFsr3Pipeline::VulkanSceneFsr3Pipeline(
        VulkanSceneFsr3Pipeline&&) noexcept = default;
    VulkanSceneFsr3Pipeline& VulkanSceneFsr3Pipeline::operator=(
        VulkanSceneFsr3Pipeline&&) noexcept = default;

    bool VulkanSceneFsr3Pipeline::init(VulkanContext& context) {
        if (!impl_)
            impl_ = std::make_unique<Impl>();
        return impl_->init(context);
    }

    VulkanSceneFsr3PipelineResult VulkanSceneFsr3Pipeline::record(
        const VkCommandBuffer command_buffer,
        const VulkanSceneFsr3PipelineRequest& request) {
        return impl_ ? impl_->record(command_buffer, request)
                     : VulkanSceneFsr3PipelineResult{
                           .status = VulkanSceneFsr3PipelineStatus::InvalidRequest,
                           .view = request.temporal.temporal.view};
    }

    void VulkanSceneFsr3Pipeline::reset(const TemporalViewId view,
                                        const TemporalResetReason reason) {
        if (impl_)
            impl_->reset(view, reason);
    }

    void VulkanSceneFsr3Pipeline::resetAll(const TemporalResetReason reason) {
        if (impl_)
            impl_->resetAll(reason);
    }

    void VulkanSceneFsr3Pipeline::releaseResources(const TemporalResetReason reason) {
        if (impl_)
            impl_->releaseResources(reason);
    }

    void VulkanSceneFsr3Pipeline::shutdown() { impl_.reset(); }

    std::size_t VulkanSceneFsr3Pipeline::residentOutputCount() const {
        if (!impl_)
            return 0;
