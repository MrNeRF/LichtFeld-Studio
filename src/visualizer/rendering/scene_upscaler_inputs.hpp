...
namespace lfs::vis {
    ...
    enum class SceneUpscalerInputIssue : std::uint32_t {
        ...
        MotionJitterMismatch = 1u << 6u,
        MissingJitter = 1u << 7u,
        MissingHistory = 1u << 8u,
        HistoryExtent = 1u << 9u,
    };

    [[nodiscard]] constexpr SceneUpscalerInputIssue operator|(
        const SceneUpscalerInputIssue lhs, const SceneUpscalerInputIssue rhs) noexcept {
    ...
    constexpr SceneUpscalerInputIssue& operator|=(SceneUpscalerInputIssue& lhs,
                                                  const SceneUpscalerInputIssue rhs) noexcept {
        lhs = lhs | rhs;
        return lhs;
    }

    [[nodiscard]] constexpr bool hasSceneUpscalerInputIssue(
        const SceneUpscalerInputIssue value, const SceneUpscalerInputIssue issue) noexcept {
