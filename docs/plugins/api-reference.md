...
| `export_viewport_image(path, format='', width=0, height=0, transparent=False, jpeg_quality=95, include_provenance=True)` | `dict` | Export active viewport to PNG/JPEG |
...
lf.export_scene(
    ...
    include_provenance: bool = True,  # ignored for COLMAP and SPZ v3
)
...
| `lf.io.save_ply(data, path, binary=True, progress=None, extra_attributes=None, include_provenance=True)` | `None` | Save `SplatData` as PLY |
| `lf.io.save_point_cloud_ply(point_cloud, path, extra_attributes=None, include_provenance=True)` | `None` | Save `PointCloud` as PLY |
| `lf.io.save_sog(data, path, kmeans_iterations=10, use_gpu=True, progress=None, include_provenance=True)` | `None` | Save SOG-compressed splats |
...
| `lf.io.save_usd(data, path, include_provenance=True)` | `None` | Save OpenUSD gaussian file |
| `lf.io.save_nurec_usdz(data, path, include_provenance=True)` | `None` | Save NuRec-compatible USDZ |
| `lf.io.export_html(data, path, kmeans_iterations=10, progress=None, include_provenance=True)` | `None` | Self-contained HTML viewer |
| `lf.io.save_image(path, image, include_provenance=True)` | `None` | Save PNG/JPG/TIFF/EXR from `[H,W,C]` or `[C,H,W]` tensor |
...
`include_provenance` is ignored for COLMAP and SPZ v3 (legacy gzip).
