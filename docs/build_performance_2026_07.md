...
seconds. The final app manifest resolves 86 packages. Large tools such as `uv`
...
| Direct Vulkan ONNX | preprocess depth/normal inference | yes | dependency-free parser and embedded compute shaders | keep in tree | Vulkan-only inference avoids a second packaged accelerator stack. |
