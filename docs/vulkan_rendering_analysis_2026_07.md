...
### P5 — dead device capabilities and release-safe RmlUi texture upload

- Commits: `91344c337` (`fix(vulkan): stop enabling unused device features`) and `1eae99135` (`fix(vulkan): harden RmlUi texture uploads`).
- Dead capability deletion: buffer device address is no longer a device-selection requirement or enabled Vulkan 1.2 feature, and VMA no longer requests its allocator flag. Descriptor indexing/bindless feature bits and runtime flag, `VK_EXT_shader_object`, extended-dynamic-state-3, and cooperative-matrix extension/feature chains, flags, accessors, and log fields were deleted. Push descriptors, float16/storage16, host image copy, synchronization2, timeline semaphore, dynamic rendering, subgroup control, shader atomic float, and swapchain maintenance remain because current code consumes them.
- RmlUi hardening: `CreateTexture` now rejects null allocator/device/sampler/data, nonpositive or overflowing dimensions, and byte-count mismatch in release builds. Image allocation, both host-copy transitions, host copy, staging allocation/map/flush, command begin/end/submit/wait/reset, and image-view creation all fail the texture cleanly with diagnostics and unwind image/view/VMA/VRAM accounting. External allocator and upload-manager partial initialization no longer continue through null handles. The staging fallback now flushes noncoherent memory explicitly.
- Dead code/comments: the generic release build no longer relies on the expression-only texture assertions for these active paths; the copy-pasted texture-upload tutorial comment was removed. The uncalled and invalid `VulkanExternalTensorStorage::cudaPtr()` API was deleted, and the exportable-block comments now describe the actual split ownership: Tensor owns the CUDA data pointer while the storage view owns Vulkan buffer offset/lifetime.
- Gates: final `cmake --build build -j8` passed. The exact focused filter ran 55 tests: 52 passed; only the same three documented lifecycle failures remained. Static `rg` found no BDA usage/allocator flag, removed capability accessor or extension name, descriptor-indexing enablement, cooperative/shader-object state, or `cudaPtr()` caller. `git diff --check` passed before both commits. An intermediate compile caught a `unique_ptr`/raw diagnostic-label argument mismatch; it was corrected before the final green build and no red commit was made.
- Measurements: device capability and texture behavior were verified statically/build-only because a GUI/device launch is forbidden. No steady-state performance claim is attached; the payoff is a smaller compatible device surface and bounded OOM/driver error paths.
- Deviation: the unreachable self-owned RmlUi instance/device/swapchain implementation was not deleted in this phase. It is a much larger deletion with shutdown/build-platform implications and remains a separate stretch change; only the application-used external backend was hardened tonight.
- Needs interactive validation: Vulkan startup/capability log on the primary GPU and a device that lacks one or more removed optional capabilities; RmlUi fonts/images and async preview replacement; host-image-copy upload on supported post-Volta hardware; forced staging fallback with host image copy disabled; allocation/map/host-copy/queue-submit failure injection; texture reload/deferred deletion; validation layers; resize; and clean shutdown.

### Final branch audit

- Branch: `vulkan-hardening`; P0 through P5 landed, no phase was parked or reverted, and no push/PR was created.
- Scope: every tracked path in `master..vulkan-hardening` is under `src/rendering/**`, `src/visualizer/**`, the CUDA/Vulkan bridge, or this report. The unrelated pre-existing untracked files remain untouched.
- Final gate: `cmake --build build -j8` passed. The exact focused 55-test filter finished 52/55 with only `ViewportFrameLifecycleServiceTest.ResizeActiveDefersFullRefreshUntilDebounceCompletes`, `PassiveWindowResizeDefersFullRefreshUntilDebounceCompletes`, and `ExplicitRefreshDeferralCompletesAfterStableFrames`; their signatures match baseline. Branch-wide `git diff --check master..vulkan-hardening` passed.
- Runtime measurements: none were taken because the contract prohibited launching the GUI. All FPS, p95, training-throughput, validation-layer, fixed-camera parity, and VRAM claims remain explicit morning gates rather than inferred wins.
- Amendment disposition: all additions were confirmed and fixed except the proposed PRESENT/UNDEFINED source-scope barrier bug. It is refuted because the acquire semaphore wait at `COLOR_ATTACHMENT_OUTPUT` orders the same-submit transition at its destination stage; presentation contributes no source access on this queue. The invariant is documented next to the empty source scope. The LOD teardown concern was confirmed, not refuted.

## Bonus round

Marker: `VULKAN-BONUS-REVIEW-2026-07-11`.

### Review basis and disposition

- Reviewed all 17 overnight commits in `master..8f7e1587d` individually as well as the aggregate 47-file diff (2,842 insertions, 1,864 deletions before bonus fixes). The pass covered batch/fence state, per-frame descriptors, sampled-resource replacement, indirect-count/radix/range edge cases, CUDA/Vulkan handoff ordering, API call sites, queue families, and shutdown paths.
- Fresh-hunt scope covered `src/rendering/**`, `src/visualizer/**`, and the interop bridge, including sequencer/film-strip, line/screen overlays, split composition/request construction, frame pacing, Mesh2Splat, shaders, and teardown. Section 3 findings and section 10 rejected designs were treated as prior art and not counted again.
- Disposition: **16 confirmed** findings, of which **14 were fixed** and **2 were deferred**; **9 hypotheses were refuted**. The fixes landed in **9 source commits**. No GUI, GPU-heavy smoke, push, PR, or test-file change was made.

### Confirmed findings

| ID | Disposition | Evidence and outcome |
|---|---|---|
| BR-01 | Confirmed, fixed | The P1 nonblocking GUI upload stream used an ordering-only wait before image and buffer copies. That skipped legacy-NULL producers and did not register allocator use, so replacing `upload_source_` could recycle storage still being read. Both paths now use `Tensor::sync_to_stream` (`cuda_vulkan_interop.cpp:526-571`, `832-866`; `de506ce26`). |
| BR-02 | Confirmed, fixed | The sibling audit found the same incomplete defect in direct VkSplat input packing and selection commit. Every packed tensor, including legacy-stream sources, now bridges and records the execution stream; selection masks do the same before current-stream mutation (`vksplat_input_packer.cpp:295-307`, `454-555`; `selection_service.cpp:1935-1949`; `899abf16d`). No ordering-only `waitForCUDAStream` call remains under `src/rendering/**` or `src/visualizer/**`. |
| BR-03 | Confirmed, fixed | P3 composition used the deferred CPU `buffers_.num_indices` as an availability bit. It is zero on the first GPU-count-driven frame even though raster work was recorded, so composition cleared a valid frame. Availability now follows the current GPU dispatch capacity and buffer validity (`vksplat_viewport_renderer.cpp:4679-4700`; `553865c7d`). |
| BR-04 | Confirmed, fixed | Selection-overlay reuse also used the one-frame-stale CPU count/capacity, and `last_render_used_macro_chain_` was published before submit succeeded. A separately invalidated/published resident capacity now describes the last submitted raster state, and chain metadata is committed only after submit (`vksplat_viewport_renderer.hpp:591-600`; `vksplat_viewport_renderer.cpp:6134-6181`, `6334-6336`, `7413-7415`; `553865c7d`). |
| BR-05 | Confirmed, fixed | P4's stale trainer callback clear occurred after minimized-frame and split-position cache returns. A later auxiliary submit could therefore call a lambda capturing a stopped/destroyed trainer. Training state and callback clearing now precede every early return (`rendering_manager_vulkan.cpp:606-627`; `cdfcf22fd`). |
| BR-06 | Confirmed, fixed | P2/P5 checked selected host writes but left seven sibling upload paths ignoring noncoherent `vmaFlushAllocation` failure: UI texture, point cloud, environment, depth, split, mesh buffer/texture, and viewport allocation writes (`vulkan_ui_texture.cpp:341-355`; `point_cloud_vulkan_renderer.cpp:141-171`; `vulkan_environment_pass.cpp:530-550`; `vulkan_depth_blit_pass.cpp:537-551`; `vulkan_split_view_pass.cpp:639-651`; `vulkan_mesh_pass.cpp:528-536`, `704-724`; `vulkan_viewport_pass.cpp:459-472`). All now fail and unwind or propagate (`9e8736faf`). |
| BR-07 | Confirmed, fixed | Depth and split transfer helpers leaked/overwrote a command buffer if fence creation failed after allocation. Partial command/fence state is now discarded before retry and a just-allocated command buffer is freed on fence failure (`vulkan_depth_blit_pass.cpp:192-221`; `vulkan_split_view_pass.cpp:503-533`; `9e8736faf`). |
| BR-08 | Confirmed, fixed | Radix downsweep guarded a partial partition's key write but unconditionally wrote the paired value at the same out-of-range destination. The value write now has the identical `elementCount` bound (`radix_sort/downsweep.comp:225-249`; `6453c65cb`). P3 made this existing shader defect a hot path for GPU-count-driven sorts. |
| BR-09 | Confirmed, fixed | Adaptive frame skipping incremented only while `< max` and tested `> max` inside that block, an impossible condition. After ten drops the counter stuck and adaptive skipping permanently disabled itself. The controller now forces one render, resets on nonadaptive/rendered paths, and resumes (`framerate_controller.cpp:38-74`; `3b2156d8c`). |
| BR-10 | Confirmed, fixed | Film-strip thumbnail demand can exceed its fixed 64-slot cache on wide/high-zoom timelines. With every slot claimed, `allocateSlot` defaulted to slot zero and aliased visible thumbnails. Renderer and RmlUi decoration counts are capped consistently, and exhausted allocation is explicit (`film_strip_renderer.cpp:33-53`, `101-106`, `261-280`; `rml_sequencer_panel_timeline.cpp:196-207`; `e355a3753`). |
| BR-11 | Confirmed, fixed | Mesh2Splat's render pass/framebuffer/layout/pipeline/sampler/descriptor pool were manually destroyed only on success and a few creation failures; cancellation or later upload/allocation failures leaked them until device teardown. A reverse-order RAII owner now covers every return (`mesh2splat.cpp:869-899`, `1647-1809`; `71d20e341`). |
| BR-12 | Confirmed, fixed | A failed Mesh2Splat command-pool creation left `device_` non-null, causing future `ensure()` calls to report success with no pool. The process-wide context was also unsynchronized across public GUI/Python/converter callers. Partial initialization now shuts down transactionally and conversion is serialized (`mesh2splat.cpp:901-915`, `1217-1232`, `1540-1547`; `71d20e341`). |
| BR-13 | Confirmed, fixed | Release builds trusted Mesh2Splat tensor shapes/dtypes, submesh spans, vertex indices, contiguity, finite attributes, and texture byte counts that were protected only by assertions or not checked at all. Malformed public inputs could read out of bounds. Extraction and texture upload now validate these invariants before dereference (`mesh2splat.cpp:486-700`, `1292-1310`; `71d20e341`). |
| BR-14 | Confirmed, fixed | Degenerate faces reached geometry-shader `normalize`/Jacobian operations and could emit NaNs; extreme resolution arithmetic could overflow before the uint32 capacity check. Degenerate triangles are skipped with double-precision relative area testing, zero normals/tangents get fallbacks, and capacity multiplication is bounded before evaluation (`mesh2splat.cpp:629-698`, `1587-1597`; `71d20e341`). |
| BR-15 | Confirmed, deferred | Line and screen-overlay commands retain clip metadata (`line_renderer.hpp:30-39`; `screen_overlay_renderer.hpp:29-45`, `58-59`), but Vulkan aggregation consumes every command without reading it (`gui_manager.cpp:548-585`, `767-810`). Sequencer lines, gizmos, and selection previews can bleed outside their panels. A correct fix needs clip-aware draw batches/dynamic scissors (including textured text) or robust CPU clipping; changing the shared aggregate blindly is too risky without GUI image/interaction validation. |
| BR-16 | Confirmed, deferred | Mesh2Splat exposes `light_dir`, `light_intensity`, and `ambient` (`core/mesh2splat.hpp:14-21`), but conversion never reads them. The fragment shader writes unlit albedo and `build_splat_data` discards its generated normal/PBR channels (`mesh2splat.cpp:390-431`, `1445-1502`, `1981-1982`). Defining a lighting model would intentionally change all conversion output and needs a reference-image contract, so this remains flagged rather than guessed. |

### Refuted hypotheses

| ID | Verdict | Evidence |
|---|---|---|
| RR-01 | Refuted | No new reset-before-submit fence hole remains. GS batches submit an unsignaled fence, wait, then reset (`gs_pipeline.cpp:565-655`); main-frame submission replaces a reset fence and retires the swapchain on submit failure (`vulkan_context.cpp:975-1063`). |
| RR-02 | Refuted | Per-frame depth/split/environment descriptors route by the retired frame slot, and mesh draw descriptors/UBOs are immutable per frame/draw (`vulkan_depth_blit_pass.cpp:298-303`, `638-666`; `vulkan_split_view_pass.cpp:278-283`, `780-803`; `vulkan_environment_pass.cpp:250-255`, `597-626`; `vulkan_mesh_pass.cpp:446-447`, `1745-1860`). No aggregate-diff call site still uses the old signatures. |
| RR-03 | Refuted | Sampled depth/split/environment/mesh and VkSplat output replacements wait submitted graphics work before destruction (`vulkan_depth_blit_pass.cpp:455-475`; `vulkan_split_view_pass.cpp:550-566`; `vulkan_environment_pass.cpp:176-191`; `vulkan_mesh_pass.cpp:1808-1822`; `vksplat_viewport_renderer.cpp:4525-4536`). |
| RR-04 | Refuted | The indirect-count control itself handles zero, first invocation, capacity clamp, and signed-overflow sentinel without reading a negative or unclamped count (`prepare_tile_sort.slang:23-64`; `gs_renderer.cpp:170-186`; `vksplat_viewport_renderer.cpp:6362-6389`). The real first-frame failures were the stale CPU-side composition/reuse gates fixed in BR-03/04. |
| RR-05 | Refuted | CUDA/Vulkan timeline enqueue ordering and monotonic bookkeeping are correct: the value is published only after `cudaSignalExternalSemaphoresAsync` succeeds (`cuda_vulkan_interop.cpp:574-593`), and GUI transition submits wait that exact value. The defect was source-allocation lifetime, not semaphore direction. |
| RR-06 | Refuted | Distinct compute/graphics families use concurrent sharing for external images/buffers, and distinct graphics/present families use concurrent swapchain sharing (`vulkan_context.cpp:2190-2216`, `2360-2384`, `2920-2936`). No missing ownership-transfer requirement was found in these paths. |
| RR-07 | Refuted | The narrowed VkSplat main-frame wait is still at every consumer's earliest stage: viewport, split, and depth outputs are first sampled by fragment shaders, and the frame submit waits at `FRAGMENT_SHADER` (`gui_manager.cpp:6434-6456`). No transfer/attachment consumer precedes it. |
| RR-08 | Refuted | Overnight LOD teardown now stops producers before unconfiguring/freeing storage (`vksplat_viewport_renderer.cpp:1962-1971`), renderer release waits pending batches/submitted frames (`1553-1576`), and no second app-exit race was found in that ownership chain. Mesh2Splat's independent raw-resource hole is BR-11. |
| RR-09 | Refuted | Full build plus static call-site audit found no missed changed API surface, stale direct-radix entry point, removed capability accessor, or active default-stream GUI upload call. `git diff --check master..HEAD` also passed after the source commits. |

### Fix commits

- `de506ce26` — `fix(vulkan): preserve CUDA upload source lifetimes`
- `553865c7d` — `fix(vulkan): track GPU-resident sort state`
- `cdfcf22fd` — `fix(viewport): clear stale trainer submit callbacks`
- `6453c65cb` — `fix(vulkan): bound radix value writes`
- `9e8736faf` — `fix(vulkan): harden host upload error paths`
- `3b2156d8c` — `fix(viewport): restore adaptive frame skipping`
- `e355a3753` — `fix(sequencer): bound film strip slot use`
- `71d20e341` — `fix(mesh2splat): harden Vulkan conversion failures`
- `899abf16d` — `fix(vulkan): retain cross-stream visualizer inputs`

### Gates

- `clang-format -i` was run on every changed C++/header file before its source commit; the GLSL file retained project formatting and was compiled by the shader build.
