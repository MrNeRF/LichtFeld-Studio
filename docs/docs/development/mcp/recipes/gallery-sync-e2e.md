# LichtFeld Studio ⇄ portal gallery E2E

`scripts/validate_gallery_sync.py` drives the real Asset Manager through LichtFeld Studio's MCP editor. It launches its own LichtFeld Studio process, Xvfb display and fresh `LFS_HOME`; local mode also migrates and seeds a temporary SQLite portal and runs its processor. It never configures, builds, or commits. An existing native build with gallery plugins and an NVIDIA-capable Linux runtime is required.

## Local release check

From the LichtFeld Studio checkout, with the portal dependencies already installed:
...
The script starts at `--display` (default `:93`) and skips occupied socket or lock paths until it finds a free display. It records the selected display in the report and never attaches to an existing X server. To require a particular display, pass both `--display :93` and `--strict-display`; occupancy then fails the run. `--strict-display` alone still may select an unused display. Install Xvfb and provide the GPU/Vulkan runtime in the runner. An inherited `VK_ICD_FILENAMES` is respected; otherwise the launcher selects an installed NVIDIA ICD from `/usr/share/vulkan/icd.d`. LichtFeld Studio uses `SDL_VIDEODRIVER=x11` and `--no-splash --log-level debug`. This is a GUI check on a virtual display, not LichtFeld Studio's CLI renderer.
...
4. Publishing closed `portable-multi` completes a new upload job, creates a ready portal scene, and shows “Up to date”. The native closed-project export must succeed without opening another document. Local mode inspects every `bundle_index.manifest.nodes[*].file`: `.sog`, `.ssog`, `.spz`, or `.ply` for LichtFeld Studio format. Both modes verify the scene through authenticated `GET /api/gallery/v1/splats/{id}`.
...
8. Killing and relaunching LichtFeld Studio restores the link and all transfer IDs from the journal, followed by authenticated refresh and restored panel links.
...
11. LichtFeld Studio and portal logs contain no `Traceback`, `[error]`, `Syntax error parsing property`, or `Missing localization key` lines. The only default exception is the specific unrelated `xdg-open: no method available for opening` failure, not all lines mentioning xdg-open.
...
No running LichtFeld Studio, native Python extension, portal, or network is needed:
