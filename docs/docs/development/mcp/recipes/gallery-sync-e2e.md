...
4. Publishing closed `portable-multi` completes a new upload job, creates a ready portal scene, and shows “Up to date”. The native closed-project export must succeed without opening another document. Local mode inspects every `bundle_index.manifest.nodes[*].file`: `.sog`, `.ssog`, `.spz`, or `.ply` for Studio format. Both modes verify the scene through authenticated `GET /api/gallery/v1/splats/{id}`.
