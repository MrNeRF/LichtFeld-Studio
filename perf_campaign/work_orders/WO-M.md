...
YOUR AREA (worker M): Ellipse tile intersection EXPERIMENT (plan 7.3): tighter ellipse-vs-tile test in FastGS forward tile counting/instancing (idea from spirulae IntersectTile.cu:54-89 — port the math, not code), behind a runtime flag. Measure instances/frame + steady_ms on both workloads; keep flag ON only if strictly faster with identical images (pixel test); else land flag OFF + numbers. Files: fastgs forward tile path only.
