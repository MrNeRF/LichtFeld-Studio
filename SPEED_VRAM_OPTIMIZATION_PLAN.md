# Speed & VRAM Optimization Plan — match the baseline's footprint, beat it on speed (2026-08-06)

Source: 8 parallel Grok deep-assessments of LichtFeld-Studio and the local baseline
checkout, cross-verified against the code by hand (key claims in `forward.cu`,
`sh_layout.cuh`, `regularization.cpp`, `adam_optimizer.hpp`, baseline `Tensor.h`/`PoolSlots.h`
were re-read directly). Full evidence reports: campaign analysis notes.
...
## 0. What the baseline actually does (verified, not folklore)

The maintainer's "memory allocation never fragments and keeps cache at minimum" claim
decomposes into three verified mechanisms (`baseline sources`, `PoolSlots.h`):
...
What the baseline does **not** have (our headroom to beat them): CUDA graphs, multi-stream
...
| Category (B/splat) | LFS | the baseline L1+FPBO | Δ |
|---|---|---|---|
...
Phase 0 builds the instrumentation that makes G1–G5 enforceable in CI, baseline-style
...
| 0.2 | **Bytes-per-splat ledger** in VRAM HUD/profiler: params / grads / optim / aux / raster-transient buckets, like the baseline's `VramCategory` report | `vram_hud_overlay.cpp`, `VramProfiler` | Ledger matches §0b table on a live run |
...
|---|---|---|---|
| 1.1 | **Persistent high-water sort buffers** in FastGS forward: keys×2, indices×2, CUB WS become grow-only members (the baseline slot-style), sized to max `n_instances` seen; drop the per-step 5× `cudaMallocAsync` + `cudaFreeAsync` | `fastgs/.../src/forward.cu:55`, `:261-300`, `:395-403` | Kills the dominant per-step allocator churn (G2), removes alloc latency + fragmentation |
...
This is the baseline-learning core. Port the *codec designs* (not the code): endpoint-exact
...
| # | Task | Design (from the baseline, verified) | Saves | Risk / gate |
|---|---|---|---|---|
| 2.1 | **SH-rest 16-bit block quantization**: store shN packed 2 B/cell + `float2` bounds per 256-splat block; decode in registers in FastGS forward/backward SH evaluation; re-encode in the fused Adam kernel after the update (single writer — same place the baseline's FPBO re-encodes) | `baseline Tensor.h:1288-1376` (`QuantizedTensor<16,256>`), `EngineForward.cpp:62-108` decode-on-load; integrates with our swizzle or replaces it | **−102 B/splat** (192→~90) — the whole param gap | Largest task. Quality gate G6 on full eval suite; the baseline ships this as default, and 16-bit endpoint-exact is conservative. Fallback flag to fp32 |
| 2.2 | **Joint `(u, log_s)` Adam codec** replacing uint8+per-primitive-scales: 16-bit/cell non-SH, 8-bit/cell SH, `float4` bounds per 256-splat block. Eliminates the 48 B/splat of fp32 scales; better SNR than our per-primitive scale (the baseline measured +20–30 dB on post-step SNR vs linear) | `baseline Tensor.h:916-1113`; our fused Adam already has the single read-modify-write site (`kernels_backward.cuh:59-96`) | **−26 B/splat**, likely quality-*positive* | Medium. Exact-trajectory won't hold (different rounding); gate on PSNR parity |
...
| 2.5 | (Stretch) **Grad-quant for non-fused paths**: where full fp32 grads still materialize (gsplat path, lazy `get_grad` consumers), use the signed-symmetric `decode(0)=0` codec (16-bit geom / 8-bit SH) | `the baseline GradQuant.cuh:1-68`; `adam_optimizer.cpp:425-447` | up to −175 B/splat on those paths | After 2.1/2.2 |

Post-Phase-2 ledger: 56 + ~90 + 0 + ~146 + 8 = **~300 B/splat** → G1 met, slightly under the baseline.

## Phase 3 — Tensor lib: zero-churn steady state (the baseline's allocator lesson, applied correctly)

We do **not** need the baseline's static-slot architecture globally — a general tensor lib needs a
...
| # | Task | Evidence | Notes |
|---|---|---|---|
| 3.1 | **Enforce G2 in CI**: after warmup, a training iteration performs 0 pool misses / driver allocs (counter from 0.1). Every remaining alloc site found becomes a bug with an owner | 0.1 harness | The single most valuable the baseline property |
...
| 4.5 | **One readback per refine**: batch the `sum().item()` / `count_nonzero` / threshold `item_as` host syncs into a single packed D2H (baseline-style `AsyncReadout` where a 1-event lag is acceptable) | `mrnf.cpp:643, 699, 733, 765` | Removes refine-time pipeline stalls |
...
Source: 4 additional Grok audits (campaign analysis notes),
...
## Phase 7 — Beat the baseline on speed (they left this on the table)

Only after G2/G3 hold (stable pointers + no steady-state allocs are prerequisites):

| # | Task | Why we win |
|---|---|---|
| 7.1 | **CUDA graph capture** of the steady-state step (the baseline: none). Zero-alloc + persistent buffers from Phases 1/3 make the step graph-capturable; densify/eval iterations run un-graphed | Launch overhead ~0 for 200+ kernels/step |
| 7.2 | **Multi-stream overlap**: GT decode/upload + loss readbacks on a copy stream overlapping compute (the baseline is single-stream, sync H2D) | Hides H2D entirely |
| 7.3 | (Experiment) **Ellipse-exact tile intersection** vs our AABB tiles — the baseline's ellipse test yields fewer instances → less sort + blend work | `the baseline IntersectTile.cu:54-89` |
...
## What we deliberately do NOT copy from the baseline
...
Source: campaign analysis notes (deep-dive of
