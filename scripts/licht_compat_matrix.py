...
BASELINE_MANIFEST_FILE_SHA256 = (
    "d91126736d4638eb3e62e7adf65626850397603b80eda185409e01dc8c7a7772"
)
...
PRODUCER_METADATA: dict[str, dict[str, object]] = {
    ...
    "selection-raw-u8.licht": {
        "producer": "ProjectDocument::save with populated RawU8 selection",
        "reproducible": True,
        "identity_set": "slim-selm-raw-u8-v1",
        "reproduction": {
            "project_uuid": "00013b32-0000-4000-8000-000000013b32",
            "file_uuid": "00013b3e-0000-4000-8000-000000013b3e",
            "commit_uuid": "00013b3c-0000-4000-8000-000000013b3c",
            "snapshot_uuid": "00013b3d-0000-4000-8000-000000013b3d",
            "creation_time_unix_ns": 1735689600000080690,
            "wallclock_unix_ns": 1735689601000080700,
        },
    },
}
...
def _register_fields(register: str, prefix: str, label: str) -> dict[str, str]:
    line = next((line for line in register.splitlines() if line.startswith(prefix)), None)
    if line is None:
        raise AssertionError(f"compatibility register lacks the {label} line")
    fields: dict[str, str] = {}
...
def _register_hashes(repository: Path) -> tuple[str, str, str]:
    register = (repository / "docs/compatibility.md").read_text(encoding="utf-8")
    baseline = _register_fields(
        register,
        "Published-grammar register line: `licht/1.0 ",
        "machine-readable 1.0 baseline",
    )
    candidate = _register_fields(
        register,
        "Current-candidate register line: `licht/1.0 ",
        "machine-readable current-candidate",
    )
    baseline_manifest = baseline.get("manifest", "")
    candidate_manifest = candidate.get("manifest", "")
    baseline_tree = baseline.get("tagged_tree", "")
    candidate_tree = candidate.get("tagged_tree", "")
    if any(
        len(value) != 64
        for value in (
            baseline_manifest,
        ...
        raise AssertionError(
            "compatibility register rows must record 64-hex manifest and tagged_tree hashes"
        )
    if baseline_tree != candidate_tree:
        raise AssertionError("baseline and candidate rows name different tagged parser trees")
    return baseline_manifest, candidate_manifest, baseline_tree
...
def write_manifest(repository: Path, note: str) -> str:
    ...
    for name in CANDIDATE_EXPECTED_ARTIFACTS:
        path = release / name
        if not path.is_file():
...
def _run_cpp_reader_job(repository: Path) -> None:
    binary = repository / "build/tests/lichtfeld_format_tests"
...
def verify_frozen_content(repository: Path, *, run_cpp_proof: bool) -> dict[str, Any]:
    ...
    rows = candidate_manifest["fixtures"]
    if candidate_manifest.get("schema") != 2:
        raise AssertionError("candidate release manifest must use content-addressed schema 2")
    if candidate_manifest.get("authority_sha") != PROVENANCE_SHA:
        raise AssertionError("candidate release manifest provenance SHA mismatch")
    if [row["path"] for row in rows] != list(CANDIDATE_EXPECTED_ARTIFACTS):
        raise AssertionError("candidate release corpus path/order differs from the eight locked artifacts")
    candidate_manifest_root = _manifest_root(rows)
    if candidate_manifest["manifest_root_sha256"] != candidate_manifest_root:
        raise AssertionError("candidate release manifest root mismatch")

    baseline_manifest_path = (
        repository / "tools/licht_inspect/baseline/release_manifest.json"
    )
    if _sha256(baseline_manifest_path) != BASELINE_MANIFEST_FILE_SHA256:
        raise AssertionError("baseline release manifest bytes differ from the P8 freeze")
    baseline_manifest = json.loads(baseline_manifest_path.read_text(encoding="utf-8"))
    baseline_rows = baseline_manifest["fixtures"]
    if baseline_manifest.get("schema") != 2:
        raise AssertionError("baseline release manifest must use content-addressed schema 2")
    if baseline_manifest.get("authority_sha") != PROVENANCE_SHA:
        raise AssertionError("baseline release manifest provenance SHA mismatch")
    if [row["path"] for row in baseline_rows] != list(BASELINE_EXPECTED_ARTIFACTS):
        raise AssertionError("baseline release manifest differs from the seven locked P8 artifacts")
    ...
    if baseline_register_root != baseline_manifest_root:
        raise AssertionError(
            "compatibility register baseline manifest root mismatch: "
            f"register={baseline_register_root} actual={baseline_manifest_root}"
        )
    if candidate_register_root != candidate_manifest_root:
        raise AssertionError(
            "compatibility register candidate manifest root mismatch: "
            f"register={candidate_register_root} actual={candidate_manifest_root}"
        )
    tagged = repository / "tools/licht_inspect/tagged/v1_0"
    ...
    return {
        ...
        "manifest_root": candidate_manifest_root,
        "baseline_manifest_root": baseline_manifest_root,
        "candidate_manifest_root": candidate_manifest_root,
...
def check_matrix(repository: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "baseline_release": len(frozen["baseline_rows"]),
        "candidate_release": len(release_paths),
