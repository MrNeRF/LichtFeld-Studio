...
TEST(SplatDecimate, GpuMatchesCpuReference) {
    for (int degree : {0, 2})
        for (size_t target : {2500u, 700u}) {
            ...
            auto cpu = cpu_candidates(host, 16, 4), gpu = gpu_candidates(device, 16, 4);
            size_t equal = 0;
            double max_error = 0;
...
TEST(SplatDecimate, ExactNeighboursOnDegenerateDistributions) {
    for (int distribution = 0; distribution < 6; ++distribution) {
        auto host = make_decimate_data(257, 0);
        auto v = host.view();
        for (size_t i = 0; i < host.n; ++i) {
            if (distribution == 0) {
                v.pos[i * 3] = float(i % 7);
                ...
                for (int a = 0; a < 3; ++a)
                    v.pos[i * 3 + a] *= distribution == 4 ? 1e-23f : 1e20f;
            }
        }
        auto cpu = cpu_candidates(host, 16, 4);
        auto gpu = gpu_candidates(transfer(host, Device::CUDA), 16, 4);
        EXPECT_EQ(cpu.idx, gpu.idx) << "distribution " << distribution;
    }
}
...
TEST(SplatDecimate, MergeMathMatchesReference) {
    // Generated by reports/decimate_reference.ts using the unmodified reference.
    ...
    for (int pair = 0; pair < 3; ++pair) {
        ...
        for (bool gpu : {false, true}) {
            auto candidates = gpu ? gpu_candidates(transfer(d, Device::CUDA), 16, 4) : cpu_candidates(d, 16, 4);
...
TEST(SplatDecimate, ExactNeighbourParityForAllHeapWidths) {
    auto d = make_decimate_data(8193, 1);
    // Exercise float-key ties and partially populated heaps as well as random data.
    for (int i = 1; i < 20; ++i)
        std::copy_n(d.view().pos, 3, d.view().pos + i * 3);
    auto device = transfer(d, Device::CUDA);
    for (int k : {1, 2, 3, 8, 16, 31, 32}) {
        const int candidates = std::min(4, k);
        auto cpu = cpu_candidates(d, k, candidates);
        auto gpu = gpu_candidates(device, k, candidates);
        EXPECT_EQ(cpu.idx, gpu.idx) << "k=" << k;
        EXPECT_EQ(cpu.cost, gpu.cost) << "k=" << k;
    }
}

TEST(SplatDecimate, GridParityAcrossCellBoundariesAndExtremeScales) {
    for (const float scale : {1.0f, 1e-20f, 1e20f}) {
        auto d = make_decimate_data(8193, 0);
        auto v = d.view();
        // Fix bounds and place probes on both sides of exact grid faces.
        for (int a = 0; a < 3; ++a) {
            v.pos[a] = -10;
            v.pos[3 + a] = 10;
        }
        ...
        for (size_t i = 0; i < d.n * 3; ++i)
            v.pos[i] *= scale;
        auto cpu = cpu_candidates(d, 16, 4);
        auto gpu = gpu_candidates(transfer(d, Device::CUDA), 16, 4);
        EXPECT_EQ(cpu.idx, gpu.idx) << "scale=" << scale;
        EXPECT_EQ(cpu.cost, gpu.cost) << "scale=" << scale;
    }
}
