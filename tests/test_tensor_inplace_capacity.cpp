...
protected:
    void SetUp() override {
        ...
        tensor_1d_ = Tensor::zeros_direct({initial_size_}, capacity_, Device::GPU);
        tensor_2d_ = Tensor::zeros_direct({initial_size_, 3}, capacity_, Device::GPU);
        tensor_3d_ = Tensor::zeros_direct({initial_size_, 4, 3}, capacity_, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, AddInplaceTensor_PreservesCapacity) {
    auto other = Tensor::full({initial_size_, 3}, 2.0f, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, SubInplaceTensor_PreservesCapacity) {
    auto other = Tensor::full({initial_size_, 3}, 0.5f, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, CopyFrom_PreservesCapacity) {
    auto src = Tensor::full({initial_size_, 3}, 99.0f, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, CopyInplace_PreservesCapacity) {
    auto src = Tensor::full({initial_size_, 3}, 77.0f, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, MaskedFillInplace_PreservesCapacity) {
    ...
    auto mask = Tensor::zeros_bool({initial_size_, 3}, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, IndexPutInplace_PreservesCapacity) {
    ...
    auto indices = Tensor::from_vector({0, 1, 2, 3, 4}, {5}, Device::GPU).to(DataType::Int32);
    auto values = Tensor::full({5, 3}, 123.0f, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, IndexPutMultiDim_PreservesCapacity) {
    ...
    auto tensor = Tensor::zeros_direct({100, 50}, 1000, Device::GPU);
    tensor.fill_(1.0f);

    auto row_idx = Tensor::from_vector({0, 1, 2}, {3}, Device::GPU).to(DataType::Int32);
    auto col_idx = Tensor::from_vector({0, 1, 2}, {3}, Device::GPU).to(DataType::Int32);
    auto values = Tensor::full({3}, 999.0f, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, ScatterInplace_PreservesCapacity) {
    auto indices = Tensor::from_vector({0, 5, 10}, {3}, Device::GPU).to(DataType::Int32);
    auto src = Tensor::full({3, 3}, 777.0f, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, IndexFillInplace_PreservesCapacity) {
    auto indices = Tensor::from_vector({0, 1, 2}, {3}, Device::GPU).to(DataType::Int32);
...
TEST_F(TensorInplaceCapacityTest, IndexCopyInplace_PreservesCapacity) {
    auto indices = Tensor::from_vector({0, 1, 2}, {3}, Device::GPU).to(DataType::Int32);
    auto src = Tensor::full({3, 3}, 444.0f, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, IndexAddInplace_PreservesCapacity) {
    auto indices = Tensor::from_vector({0, 1, 2}, {3}, Device::GPU).to(DataType::Int32);
    auto src = Tensor::full({3, 3}, 333.0f, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, AppendGatherCopiesSelectedRowsExactly) {
    ...
    const auto indices = Tensor::from_vector(
                             std::vector<int>{0, 1, 0}, {3}, Device::GPU)
                             .to(DataType::Int32);
...
TEST_F(TensorInplaceCapacityTest, ChainedOperations_PreserveCapacity) {
    ...
    auto more_indices = Tensor::from_vector({0, 1, 2, 3, 4}, {5}, Device::GPU).to(DataType::Int32);
...
TEST_F(TensorInplaceCapacityTest, IndexPut_WithInt64Indices_PreservesCapacity) {
    ...
    auto indices = Tensor::from_vector({0, 1, 2}, {3}, Device::GPU).to(DataType::Int64);
    auto values = Tensor::full({3, 3}, 888.0f, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, MultipleIndexPuts_PreservesCapacity) {
    ...
    for (int i = 0; i < 10; i++) {
        auto indices = Tensor::from_vector({i * 5, i * 5 + 1, i * 5 + 2}, {3}, Device::GPU).to(DataType::Int32);
        auto values = Tensor::full({3, 3}, static_cast<float>(i), Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, IndexPut3D_PreservesCapacity) {
    auto indices = Tensor::from_vector({0, 1, 2}, {3}, Device::GPU).to(DataType::Int32);
    auto values = Tensor::full({3, 4, 3}, 111.0f, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, ViewOperations_DoNotPreserveCapacity) {
    ...
    auto tensor_with_dim1 = Tensor::zeros_direct({initial_size_, 1, 3}, capacity_, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, DANGER_SelfAssignmentThroughView_LosesCapacity) {
    ...
    auto test_tensor = Tensor::zeros_direct({initial_size_, 3}, capacity_, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, SAFE_CopyFromPreservesCapacity) {
    ...
    auto source = Tensor::full({initial_size_, 3}, 42.0f, Device::GPU);
...
TEST_F(TensorInplaceCapacityTest, REGRESSION_IndexPut_ThenAppendZeros) {
    ...
    auto means = Tensor::zeros_direct({n_gaussians, 3}, max_capacity, Device::GPU);
    means.fill_(1.0f);

    // Simulate fill_free_slots_with_data() calling index_put_()
    auto free_indices = Tensor::from_vector({50, 51, 52, 53, 54}, {5}, Device::GPU).to(DataType::Int64);
    auto new_data = Tensor::full({5, 3}, 99.0f, Device::GPU);
