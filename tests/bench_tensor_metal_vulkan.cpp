...
namespace {
    ...
    // Median wall time of one run, which ends when the returned tensor's work completed.
    double median_ms(const GpuBackend backend, const std::function<Tensor()>& run) {
...
int main() {
    ...
    for (const size_t count : {size_t{4096}, size_t{1} << 20, size_t{1} << 24}) {
        ...
        const auto compare = [&](const char* const name, const std::function<Tensor(const Inputs&)>& op) {
            report(name, count, median_ms(GpuBackend::Vulkan, [&] { return op(vulkan); }),
                   median_ms(GpuBackend::Metal, [&] { return op(metal); }));
        };
