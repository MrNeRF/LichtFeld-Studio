...
namespace {
    ...
    TEST(CheckpointParamsJsonTest,
        ...
        auto source_model = make_checkpoint_test_splat(
            2, lfs::core::Device::CUDA);
        lfs::training::MCMC source_strategy(*source_model);
        source_strategy.initialize(params.optimization);
        ASSERT_TRUE(lfs::test::write_checkpoint_fixture(
        ...
        lfs::training::MCMC target_strategy(*target_model);
        auto resumed_params = make_params_json_test_params(temp_dir);
        target_strategy.initialize(resumed_params.optimization);
        ...
        auto trainer_params = *path_params;
        trainer_params.dataset.data_path = temp_dir;
        trainer_params.resume_checkpoint = checkpoint;
        lfs::core::Scene resumed_scene;
        add_checkpoint_test_camera(resumed_scene);
        resumed_scene.addSplat("Model", make_checkpoint_test_splat(2, lfs::core::Device::CUDA));
        resumed_scene.setTrainingModelNode("Model");
        lfs::training::Trainer trainer(resumed_scene);
        const auto initialized_trainer = trainer.initialize(trainer_params);
        ASSERT_TRUE(initialized_trainer.has_value()) << initialized_trainer.error();
        // initialize() must reach its automatic legacy checkpoint restore;
        // testing the loader alone would miss an early runtime rejection.
        EXPECT_EQ(trainer.get_current_iteration(), 5);
        EXPECT_TRUE(trainer.getParams().optimization.gut);
        EXPECT_TRUE(trainer.getParams().optimization.undistort);
        EXPECT_FALSE(trainer.getParams().optimization.mip_filter);
        EXPECT_FALSE(trainer.getParams().optimization.use_depth_loss);
        EXPECT_FALSE(trainer.getParams().optimization.use_normal_loss);
        trainer.shutdown();

        // Exercise the embedded CKPT installation path as well, using a
        // synthetic camera/model rather than an external training dataset.
        std::ostringstream payload(std::ios::out | std::ios::binary);
        ASSERT_TRUE(lfs::training::serialize_checkpoint(
            payload, 5, source_strategy, params,
            nullptr, nullptr, nullptr, nullptr));
        const auto payload_bytes = payload.str();
        std::vector<std::byte> bytes(payload_bytes.size());
        std::memcpy(bytes.data(), payload_bytes.data(), bytes.size());
        const auto checkpoint_uuid = lfs::core::generate_uuid_v4();
        auto chunk = lfs::io::project::LazyChunkValue::from_owned(
            std::move(bytes), checkpoint_uuid);
        ASSERT_TRUE(chunk) << lfs::format_for_developer(chunk.error());
        auto document = lfs::io::project::ProjectDocument::create(
            lfs::core::generate_uuid_v4());
        ASSERT_TRUE(document) << lfs::format_for_developer(document.error());
        ASSERT_TRUE(document->set_checkpoint(checkpoint_uuid, std::move(*chunk)));
        auto project_params = trainer_params;
        project_params.resume_checkpoint.reset();
        project_params.resume_project = temp_dir / "legacy.licht";
        lfs::core::Scene project_scene;
        add_checkpoint_test_camera(project_scene);
        project_scene.addSplat("Model", make_checkpoint_test_splat(2, lfs::core::Device::CUDA));
        project_scene.setTrainingModelNode("Model");
        auto installed = lfs::training::installTrainerFromProjectCheckpoint(
            project_scene, *document, checkpoint_uuid, project_params,
            "legacy.licht", 5);
        ASSERT_TRUE(installed) << installed.error();
        const auto restored_params = installed->trainer->getParams();
        EXPECT_TRUE(restored_params.optimization.gut);
        EXPECT_TRUE(restored_params.optimization.undistort);
        EXPECT_FALSE(restored_params.optimization.mip_filter);
        EXPECT_FALSE(restored_params.optimization.use_depth_loss);
        EXPECT_FALSE(restored_params.optimization.use_normal_loss);
        EXPECT_EQ(installed->trainer->get_current_iteration(), 5);

        const auto resaved_dir = temp_dir / "resaved";
        ASSERT_TRUE(lfs::test::write_checkpoint_fixture(
            resaved_dir, 5, installed->trainer->get_strategy(), restored_params,
            nullptr, nullptr, nullptr, nullptr));
        const auto resaved = lfs::core::load_checkpoint_params(
            lfs::test::checkpoint_fixture_path(resaved_dir));
        ASSERT_TRUE(resaved) << resaved.error();
        EXPECT_EQ(resaved->optimization.to_json(), restored_params.optimization.to_json());
        const auto unchanged = lfs::core::load_checkpoint_params(checkpoint);
        ASSERT_TRUE(unchanged) << unchanged.error();
        EXPECT_EQ(unchanged->optimization.to_json(), path_params->optimization.to_json());
        installed->trainer->shutdown();

        auto explicit_undistort_model = make_checkpoint_test_splat(1, lfs::core::Device::CUDA);
        ...
        explicit_undistort_strategy.initialize(explicit_undistort_params.optimization);
        ...
        auto use_3dgs_model = make_checkpoint_test_splat(1, lfs::core::Device::CUDA);
        lfs::training::MCMC use_3dgs_strategy(*use_3dgs_model);
        auto use_3dgs_params = make_params_json_test_params(temp_dir);
        use_3dgs_strategy.initialize(use_3dgs_params.optimization);
        use_3dgs_params.overrides.optimization_json = R"({"gut":false})";
        const auto use_3dgs_resume = lfs::training::load_checkpoint(
            checkpoint, use_3dgs_strategy, use_3dgs_params,
            nullptr, nullptr, nullptr, nullptr);
        ASSERT_TRUE(use_3dgs_resume.has_value()) << use_3dgs_resume.error();
        EXPECT_FALSE(use_3dgs_params.optimization.gut);
        EXPECT_TRUE(use_3dgs_params.optimization.mip_filter);
        EXPECT_TRUE(use_3dgs_params.optimization.use_depth_loss);
        EXPECT_TRUE(use_3dgs_params.optimization.use_normal_loss);
        ...
        EXPECT_NE(explicit_resume.error().find("Depth Loss"),
                  std::string::npos);

        const auto gs_checkpoint_dir = temp_dir / "3dgs";
        auto gs_params = make_params_json_test_params(gs_checkpoint_dir);
        gs_params.optimization.use_depth_loss = true;
        auto gs_source_model = make_checkpoint_test_splat(2);
        lfs::training::MCMC gs_source_strategy(*gs_source_model);
        ASSERT_TRUE(lfs::test::write_checkpoint_fixture(
                        gs_checkpoint_dir, 5, gs_source_strategy, gs_params,
                        nullptr, nullptr, nullptr, nullptr)
                        .has_value());
        auto gut_override_params = make_params_json_test_params(gs_checkpoint_dir);
        gut_override_params.overrides.optimization_json = R"({"gut":true})";
        auto gut_override_model = make_checkpoint_test_splat(1);
        lfs::training::MCMC gut_override_strategy(*gut_override_model);
        const auto gut_override_resume = lfs::training::load_checkpoint(
            lfs::test::checkpoint_fixture_path(gs_checkpoint_dir),
            gut_override_strategy, gut_override_params,
            nullptr, nullptr, nullptr, nullptr);
        ASSERT_FALSE(gut_override_resume.has_value());
        EXPECT_NE(gut_override_resume.error().find("Depth Loss"),
                  std::string::npos);
