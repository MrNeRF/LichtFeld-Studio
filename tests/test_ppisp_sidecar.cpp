...
namespace {
    ...
    TEST_F(PPISPSidecarTest, SaveLoadWithMetadataSupportsRemappedImport) {
        ...
        auto save_result = lfs::training::save_ppisp_file(path, source, nullptr, &metadata);
        ASSERT_TRUE(save_result) << save_result.error();

        PPISP loaded(1);
        PPISPFileMetadata loaded_metadata;
        auto load_result = lfs::training::load_ppisp_file(path, loaded, nullptr, &loaded_metadata);
