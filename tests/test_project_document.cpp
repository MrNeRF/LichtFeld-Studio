...
namespace {
    ...
    TEST(ProjectDocumentTest,
         EmbeddedLegacy3DGutCheckpointRoundTripPreservesStoredOptions) {
        const auto training_uuid = fixed_uuid(19'200);
        const auto checkpoint_uuid = fixed_uuid(19'201);
        ...
        parameters.optimization.gut = true;
        parameters.optimization.undistort = true;
        parameters.optimization.mip_filter = true;
        parameters.optimization.use_depth_loss = true;
        parameters.optimization.use_normal_loss = true;
        ...
        auto document = make_empty_document(fixed_uuid(19'202), 100);
        require_status(document->edit_scene_graph().upsert_node(
            SceneNodeRecord{
                .uuid = training_uuid,
                .type = "splat",
        ...
        ASSERT_TRUE(staged->report().checkpoint_params.has_value());
        const auto& restored =
            staged->report().checkpoint_params->optimization;
        EXPECT_TRUE(restored.gut);
        EXPECT_TRUE(restored.undistort);
        EXPECT_TRUE(restored.mip_filter);
        EXPECT_TRUE(restored.use_depth_loss);
        EXPECT_TRUE(restored.use_normal_loss);
    }
