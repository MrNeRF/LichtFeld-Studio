...
bool CrateReader::GetUncompressedArrayData(const crate::ValueRep &rep,
                                           FlatArrayData *out) const {
  if (!out || !rep.IsArray() || rep.IsInlined() || rep.IsCompressed() ||
  ...
  if (VERSION_LESS_THAN_0_8_0(_version)) {
    ...
    count = legacy_count;
  } else if (!_sr->read8(&count)) {
    return false;
  }
  if (count > _config.maxArrayElements) {
    return false;
  }

  std::size_t component_size = 0;
  switch (static_cast<crate::CrateDataTypeId>(rep.GetType())) {
  ...
  const std::size_t components = rep.GetType() == static_cast<int32_t>(crate::CrateDataTypeId::CRATE_DATA_TYPE_QUATH) ||
                                         rep.GetType() == static_cast<int32_t>(crate::CrateDataTypeId::CRATE_DATA_TYPE_QUATF) ||
                                         rep.GetType() == static_cast<int32_t>(crate::CrateDataTypeId::CRATE_DATA_TYPE_QUATD)
                                     ? 4
                                     : rep.GetType() == static_cast<int32_t>(crate::CrateDataTypeId::CRATE_DATA_TYPE_VEC3H) ||
                                               rep.GetType() == static_cast<int32_t>(crate::CrateDataTypeId::CRATE_DATA_TYPE_VEC3F) ||
                                               rep.GetType() == static_cast<int32_t>(crate::CrateDataTypeId::CRATE_DATA_TYPE_VEC3D)
                                         ? 3
                                         : 1;
  if (count > std::numeric_limits<std::size_t>::max() / (component_size * components) ||
      count * component_size * components > _sr->size() - _sr->tell()) {
    return false;
  }
  out->data = _sr->data() + _sr->tell();
  out->count = count;
  return true;
}

bool CrateReader::UnpackValueRepForFlat(const crate::ValueRep &rep,
                                        crate::CrateValue *value) {
  return UnpackValueRep(rep, value);
}
